...
| [SILA2_INTEGRATION.md](SILA2_INTEGRATION.md) | SiLA2 standard, FDL format, sila2 library, execution strategies |
| [SERVERS.md](SERVERS.md) | Per-server command reference: Opentrons, Tecan, Mobile Robot, Manual Station |
| [API_REFERENCE.md](API_REFERENCE.md) | REST endpoints, WebSocket events, gRPC SiLA2 interface |
...
**sila2 Library**: All current servers are built with the `sila2` Python library. The orchestrator tries execution strategies in order: **Strategy 0** — `SilaClient` (standard SiLA2, preferred, used by all current servers); **Strategy 1** — legacy `SiLA2Common` fallback; **Strategy 2** — dynamic stub loading. New servers automatically get Strategy 0.
