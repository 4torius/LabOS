...
namespace TecanSiLA2Server
{
    public class Program
    {
        ...
        /// </summary>
        // i-control installation path — native USB DLL lives here
        private static readonly string IControlPath =
            @"C:\Program Files (x86)\Tecan\i-control\2.0";

        [STAThread]
        public static void Main(string[] args)
        {
            // Ensure the native COM USB DLL (Communication.Port.USB.Unmanaged.dll)
            // and all managed SDK DLLs are found at runtime.
            if (Directory.Exists(IControlPath))
            {
                var path = Environment.GetEnvironmentVariable("PATH") ?? "";
                if (!path.Contains(IControlPath))
                    Environment.SetEnvironmentVariable("PATH", IControlPath + ";" + path);
            }

            AppDomain.CurrentDomain.AssemblyResolve += (_, e) =>
            {
                var name = new System.Reflection.AssemblyName(e.Name).Name + ".dll";
                var candidate = Path.Combine(IControlPath, name);
    ...
    public class SiLA2ServerHostedService : IHostedService
    {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            // mDNS: disabled when running as internal bridge (Python SiLA2 server handles discovery)
            ...
            {
                _logger.LogInformation("[mDNS] Disabilitato (bridge interno)");
            }
        }
