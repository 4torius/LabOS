#!/usr/bin/env python3
"""
ManualStation SiLA2 Server — Main Entry Point (sila2 standard library)
======================================================================

Virtual station for operator interactions in automated workflows.
Uses the official sila2 Python library (v0.14+) instead of raw gRPC.

Usage:
    python main.py                       # default config.yaml, insecure
    python main.py --port 50360          # override port
    python main.py --secure              # enable TLS (self-signed cert)
    python main.py --no-discovery        # disable mDNS
"""

import argparse
import contextlib
import logging
import os
import signal
import sys
from pathlib import Path
from uuid import UUID, uuid4

# Add server root to sys.path so that absolute imports work for both
# "generated.*" and "src.*" packages.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

import yaml
from sila2.server import SilaServer

from generated.manualstation.manualstation_feature import ManualStationFeature
from src.manual_station_impl import ManualStationImpl


class _SuppressSubscriptionManagerLogs(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        return not record.name.startswith("SubscriptionManagerThread")


def _load_config(path: str) -> dict:
    cfg_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), path)
    if os.path.exists(cfg_path):
        with open(cfg_path) as f:
            return yaml.safe_load(f) or {}
    return {}


def _get_persistent_uuid(server_dir: str) -> UUID:
    uuid_file = Path(server_dir) / ".server_uuid"
    if uuid_file.exists():
        return UUID(uuid_file.read_text().strip())
    new_uuid = uuid4()
    uuid_file.write_text(str(new_uuid))
    return new_uuid


def main() -> int:
    parser = argparse.ArgumentParser(
        description="ManualStation SiLA2 Server — operator task management"
    )
    parser.add_argument("--config", "-c", default="config.yaml")
    parser.add_argument("--host", default=None)
    parser.add_argument("--port", "-p", type=int, default=None)
    parser.add_argument(
        "--webapp-url",
        default=os.environ.get("WEBAPP_URL", "http://127.0.0.1:5000"),
    )
    parser.add_argument(
        "--secure",
        action="store_true",
        default=False,
        help="Enable TLS (auto-generates a self-signed certificate)",
    )
    parser.add_argument(
        "--no-discovery",
        action="store_true",
        default=False,
        help="Disable mDNS/DNS-SD auto-discovery",
    )
    args = parser.parse_args()

    config = _load_config(args.config)
    host = args.host or config.get("server", {}).get("host", "0.0.0.0")
    port = args.port or config.get("server", {}).get("port", 50360)

    log_level = config.get("logging", {}).get("level", "INFO")
    logging.basicConfig(
        level=getattr(logging, log_level, logging.INFO),
        format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    )
    for handler in logging.root.handlers:
        handler.addFilter(_SuppressSubscriptionManagerLogs())
    logging.getLogger("SiLAService").setLevel(logging.WARNING)
    logging.getLogger("sila2").setLevel(logging.WARNING)
    logging.getLogger("grpc").setLevel(logging.WARNING)

    # ── Build the SiLA2 standard server ──────────────────────────────────────
    sila_server = SilaServer(
        server_name="ManualStation",
        server_type="OperatorStation",
        server_description="Virtual operator station for manual tasks in automated workflows",
        server_version="1.0",
        server_vendor_url="https://bicocca.lab",
        server_uuid=_get_persistent_uuid(os.path.dirname(os.path.abspath(__file__))),
    )

    impl = ManualStationImpl(sila_server, webapp_url=args.webapp_url)
    sila_server.set_feature_implementation(ManualStationFeature, impl)

    # ── Start ─────────────────────────────────────────────────────────────────
    enable_discovery = not args.no_discovery

    if args.secure:
        sila_server.start(host, port, enable_discovery=enable_discovery)
        if sila_server.generated_ca:
            ca_path = "generated_ca.pem"
            with open(ca_path, "wb") as fp:
                fp.write(sila_server.generated_ca)
            print(f"  CA cert: {ca_path}")
    else:
        sila_server.start_insecure(host, port, enable_discovery=enable_discovery)

    print()
    print("=" * 60)
    print("  MANUAL STATION - SiLA2 Server (standard library)")
    print("=" * 60)
    print(f"  gRPC   : {host}:{port}")
    print(f"  Mode   : {'TLS' if args.secure else 'insecure'}")
    print(f"  mDNS   : {'enabled' if enable_discovery else 'disabled'}")
    print(f"  WebApp : {args.webapp_url}")
    print()
    print("  Handles: operator tasks, tip-refill notifications,")
    print("           quality checkpoints, manual confirmations.")
    print()
    print("  Waiting for tasks...")
    print("=" * 60)
    print()

    # ── Graceful shutdown ─────────────────────────────────────────────────────
    signal.signal(signal.SIGTERM, lambda *_: sila_server.grpc_server.stop(0))

    try:
        with contextlib.suppress(KeyboardInterrupt):
            sila_server.grpc_server.wait_for_termination()
    finally:
        sila_server.stop()
        print("\nServer stopped.")

    return 0


if __name__ == "__main__":
    sys.exit(main())
