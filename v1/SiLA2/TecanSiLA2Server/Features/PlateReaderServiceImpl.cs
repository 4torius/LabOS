...
namespace TecanSiLA2Server.Features
{
    ...
    public class PlateReaderServiceImpl : PlateReaderService.PlateReaderServiceBase
    {
        ...
        public override async Task RunMeasurement(
            ...
            ServerCallContext context)
        {
            var plateType = string.IsNullOrWhiteSpace(request.PlateType) ? null : request.PlateType.Trim();
            _logger.LogInformation("RunMeasurement command received: Protocol={Protocol}, PlateID={PlateId}, PlateType={PlateType}",
                request.ProtocolFile, request.PlateId, plateType ?? "(default)");
